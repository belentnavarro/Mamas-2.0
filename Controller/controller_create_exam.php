...
if (isset($_REQUEST['updateExamPage'])) {
    // Recupero el examen
    $idExamUpdate = intval($_REQUEST['idExman']);
    
    // Guardo el id del examen a modificar en la memoria
    $_SESSION['idExamUpdate'] = $idExamUpdate;
    
    header('Location: ../View/update_exam.php');
...
// Boton para quitar una pregunta cuando se modifica un examen
if (isset($_REQUEST['deleteQuestionUpdate'])){
    // Cogo las variables necesarias de la sesion y el id de la respuesta
    $examQuestionsUpdate = $_SESSION['examQuestionsUpdate'];
    $idQuestion = intval($_REQUEST['deleteIdQuestion']);
    
    foreach ($examQuestionsUpdate as $key => $value) {
        $o = $examQuestionsUpdate[$key];
